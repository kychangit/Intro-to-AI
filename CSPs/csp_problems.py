...
def sudokuCSP(initial_sudoku_board, model='neq'):
    ...
    for row in var_array:
        if model == 'neq':
            ...
            constraint_list.extend([AllDiffConstraint("Row " + str(row), row)])
    ...
    for colj in range(len(var_array[0])):
        ...
        if model == 'neq':
            ...
            constraint_list.extend([AllDiffConstraint("Col " + str(colj), scope)])
    ...
    for i in [0, 3, 6]:
        for j in [0, 3, 6]:
            ...
            if model == 'neq':
                ...
                constraint_list.extend([AllDiffConstraint("Subsquare " + str(i) + str(j), scope)])
...
def planeCSP(planes_problem):
    ...
    for i, plane in enumerate(planes):
        ...
        for j in range(len(can_fly)):
            ...
            if j == 0:
                dom = [x for x in planes_problem.can_start(plane) if x in can_fly] + [-1]
    ...
    for i in range(len(var_array)):
        for j in range(len(var_array[i]) - 1):
            ...
            constraint_list.append(TableConstraint("Follow {}{}".format(i, j), scope, flights_can_follow))
    # maintenance
    for i in range(len(var_array)):
        for j in range(len(var_array[i]) - min_maintenance_frequency + 1):
            ...
            constraint_list.append(NValuesConstraint("Maintenance {}{}".format(i, j), scope,
                                                     maintenance_flights + [-1], 1, min_maintenance_frequency))
    # Exactly one flight
    vars = [var for row in var_array for var in row]
    for flight in flights:
        constraint_list.append(NValuesConstraint("Exactly One {}".format(flight), vars,
                                                 [flight], 1, 1))
...
def solve_planes(planes_problem, algo, allsolns,
    ...
    csp, planes_flights = planeCSP(planes_problem)  # set to to your CSP
    ...
    for solution in solutions:
        temp = 0
        schedule = []
        for i in range(len(planes_flights)):
            planes = [planes_problem.planes[i]]
            for j in range(len(planes_flights[i])):
                if solution[temp][1] != -1:
                    planes.append(solution[temp][1])
                temp += 1
            schedule.append(planes)
        converted_solutions.append(schedule)
