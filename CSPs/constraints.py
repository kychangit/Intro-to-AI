...
class NValuesConstraint(Constraint):
    ...
    def check(self):
        ...
        for v in self.scope():
            if v.isAssigned():
                ...
                return True
        return self._lb <= total <= self._ub
    ...
    def hasSupport(self, var, val):
        ...
        def valsBounded(l):
            '''tests a list of assignments which are pairs (var,val)
               to see if they can satisfy the NValues'''
            total = 0
            for (var, val) in l:
        ...
        def valsUpBounded(l):
            ...
            for (var, val) in l:
                if val in self._required:
                    total += 1
            return total <= self._ub

        varsToAssign = self.scope()
        varsToAssign.remove(var)
        x = findvals(varsToAssign, [(var, val)], valsBounded, valsUpBounded)
        return x
