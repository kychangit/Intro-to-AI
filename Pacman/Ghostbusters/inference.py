...
class ParticleFilter(InferenceModule):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for particles in self.particles:
            dist[particles] += self.getObservationProb(observation, pacman, particles, jail)
        if dist.total() != 0:
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for particles in self.particles:
            ...
            for i in range(self.numGhosts):
                prob *= self.getObservationProb(observation[i], pacman, particles[i], self.getJailPosition(i))
            dist[particles] += prob
        if dist.total() != 0:
            self.particles = [dist.sample() for i in range(self.numParticles)]
        else:
